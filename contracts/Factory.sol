...
contract ERC1155Factory is ERC1155,Ownable,ReentrancyGuard,ERC2981PerTokenRoyalties{
    ...
    */
    modifier ownersOnly(uint256 _tokenId) {
        require(balanceOf(msg.sender,_tokenId) > 0, "ERC1155Tradable#ownersOnly: ONLY_OWNERS_ALLOWED");
        _;
    }
    constructor(
        string memory _name,
    ...
    //최초 nft 발행
    function mint_byOtherUsers_1st(uint256 _amount, string memory _uri, bool _nft) public allowed(msg.sender){
    ...
    //추가 nft 발행
    function mint_additionalByOtherusers(uint256 _tokenId,uint256 _amount) public creatorOnly(_tokenId) allowed(msg.sender){
        require(tokenNFT[_tokenId]==false,"NFT is not able to re-generate");
        _mint(msg.sender, _tokenId, _amount,"");
        tokenSupply[_tokenId]+=_amount;
    }
    // 배치 nft 발행 처음 발행
    function mint_byAnyOneBatch_1st(uint256 _numberOfgenerates, uint256 [] memory _amounts, bool [] memory _nfts, string [] memory _uris) public allowed(msg.sender){
        require(_numberOfgenerates>0,"at least more than 0 creation");
        require(_numberOfgenerates==_nfts.length && _numberOfgenerates==_uris.length,"length of array is not matched");
    ...
    // 배치 nft 발행. 추가 발행
    function mint_byAnyOneForAdditional_Batch(uint256 [] memory _tokenIds, uint256 [] memory _amounts) public allowed(msg.sender){
        ...
        for (uint256 i=0; i < _tokenIds.length;i++){
            ...
            require(creators[_tokenIds[i]]==msg.sender,"you are not the token creator"); //실행자가 토큰 원작자인지 확인
            require(tokenNFT[_tokenIds[i]]==false,"you are not able to re-generate the token because it is NFT"); //토큰이 NFT인지 확인.(재발행시)
    ...
    //only token Owner
    function burn(uint256 _tokenId,uint256 _burnAmount) public ownersOnly(_tokenId){
    ...
    // onlye token Creator
    function _setRoyalty(uint256 _tokenId,uint16 _royalties) public creatorOnly(_tokenId){
