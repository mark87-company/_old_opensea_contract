    ...
    Ownable
{
    ...
    using SafeMath for uint256;
    mapping(uint256 => address) mapOriginalOwner;
    mapping(uint256 => uint256) mapRoyalty;
    address salesAddress;
    event logRoyaty(uint256 indexed _tokenId, uint256 indexed Royalty_rate);
    ...
    modifier onlyContract() {
        require(
            salesAddress != address(0) && msg.sender == salesAddress,
            "only contract able to execute"
        );
        _;
    }

    //royalty setup should be set as 10000 as 100%, 0 as 0%, 1000 as 10%, 100 as 1%
    function setRoyalty(uint256 _tokenId, uint256 _setUpRate)
        public
        whenNotPaused
    {
        require(
            mapOriginalOwner[_tokenId] == msg.sender,
            "only tokenOwner can set up the royalty"
        );
    ...
    function mint_one(string memory _uri) public onlyMinter whenNotPaused {
        uint256 tokenId = totalSupply();
        mintWithTokenURI(msg.sender, tokenId.add(1), _uri);
        mapOriginalOwner[tokenId.add(1)] = msg.sender;
    }

    //only owner can create NFT for user
    function mint_by_owner(address _to, string memory _uri) public onlyOwner {
        uint256 tokenId = totalSupply();
        mintWithTokenURI(_to, tokenId.add(1), _uri);
        mapOriginalOwner[tokenId.add(1)] = _to;
    }

    function forcedTransfer(address _to, uint256 _tokenId)
        external
        onlyContract
    {
        require(_exists(_tokenId), "token dose not exists");
        address from = ownerOf(_tokenId);
        _forcedTransfer(from, _to, _tokenId);
    }

    function _setSalesAddress(address _salesAddress) public onlyOwner {
        salesAddress = _salesAddress;
    }

    function _getSalesAddress() public view returns (address) {
        return salesAddress;
    }

    //only by the contract owner
    function setRoyaltyByContract(uint256 _tokenId, uint256 _setUpRate)
        public
        onlyOwner
    {
        mapRoyalty[_tokenId] = _setUpRate;
